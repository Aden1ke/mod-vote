...
forms.post('/cast-vote-submit', async (c) => {
    ...
    if (!postId || !choice) {
        return c.json<UiResponse>({ showToast: '❌ Missing vote data.' }, 200);
    }
    ...
    if (meta.status === 'open' && Date.now() > meta.deadline) {
        ...
        return c.json<UiResponse>(
            {
                showToast:
                    '⏰ Vote deadline passed — results posted to modmail.',
            },
            200
        );
    }
    ...
    const modeNote = meta.anonymous
        ? '(anonymous)'
        : `(recorded as u/${username})`;

    return c.json<UiResponse>(
        {
            showToast: `Vote cast ${modeNote}: ${label} — Remove: ${tally.remove} | Keep: ${tally.keep} | Discuss: ${tally.discuss}`,
        },
        200
    );
});
